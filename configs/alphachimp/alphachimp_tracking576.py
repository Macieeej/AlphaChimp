...
dataset_type = 'mmaction.ChimpDataset'
file_root = '.'
...
ann_file_val = f'{anno_root}/../test_fix.json'
...
train_pipeline = [
    ...
    dict(type='RandomRescale', scale_range=(576, 576)),
    dict(type='RandomCrop', size=576),
    dict(type='Flip', flip_ratio=0.5),
    dict(type='FormatShape', input_format='NCTHW', collapse=False,pair=True),
    dict(type='PackActionInputs_PAIR',collect_keys=['imgs','imgs_pair'],padding=True,max_person=24)
]
...
val_evaluator = dict(
    type='mmdet.MOTChallengeMetric',
    metric=['HOTA', 'CLEAR', 'Identity'],
    postprocess_tracklet_cfg=[
        dict(type='mmdet.InterpolateTracklets', min_num_frames=5, max_num_frames=20)
    ])
...
optim_wrapper = dict(
    optimizer=dict(
        type='Adam',
        # params='model.track_head.parameters()',
        lr=0.0001,  # 0.0002 for DeformDETR
        weight_decay=0.0001),
    clip_grad=dict(max_norm=0.1, norm_type=2),
    paramwise_cfg=dict(custom_keys={'ana_model': dict(lr_mult=0.0)})
