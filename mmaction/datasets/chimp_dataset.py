...
class ChimpDataset(BaseActionDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for idx in range(num_samples):
            ...
            if 'old_behaviors' in ann.keys():
                old_behaviors = np.array(ann['old_behaviors'])
            else:
                old_behaviors = np.zeros(1)
            ...
            ann = dict(
                gt_bboxes= ann_img['entity_box'], gt_labels=ann_img['label'],label_mask = ann_img['label_mask'],entity_ids=ann_img['entity_id'],old_behaviors=ann_img['old_behaviors'])
            frame_dir = video_id
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        if len(ann['entity_ids']) == 0:
            data_info['gt_bboxes'] = np.zeros((1,4))
            data_info['gt_labels'] = np.zeros((1,self.num_classes))
            ann['label_mask'] = np.zeros(1)
        else:
        ...
        data_info['old_label'] = ann['old_behaviors']
