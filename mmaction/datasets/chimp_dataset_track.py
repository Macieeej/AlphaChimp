...
class ChimpDataset_Track(BaseActionDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for idx in range(num_samples):
            ...
            video_info = dict(
                ...
                entity_id=entity_id,
                shot_info=shot_info,
                num_chimp=len(ann['entity_id']))
            records_dict_by_img[img_key] = video_info
            video_id, timestamp = img_key.split(',')
            ann_img = records_dict_by_img[img_key]
            ann = dict(
                gt_bboxes= ann_img['entity_box'], gt_labels=ann_img['label'],entity_ids=ann_img['entity_id'],num_chimp=ann_img['num_chimp'])
            frame_dir = video_id
    ...
    def filter_data(self) -> List[dict]:
        ...
        data_list = data_list[2320:]
        num_samples = len(data_list)
        video_idx = defaultdict(list)
        for idx in range(num_samples):
            video_idx[data_list[idx]['video_id']].append(idx)
        pred = -1
        for vkey in video_idx.keys():
            video_idx[vkey] = np.array(video_idx[vkey])
            for idx in video_idx[vkey]:
                assert pred < idx,f"{pred},{idx},{vkey}"
                pred = idx
        for idx in range(num_samples):
            video_range = np.array(video_idx[data_list[idx]['video_id']])
            data_list[idx]['ann']['video_range'] = [video_range[0].tolist(),video_range[-1].tolist()]
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        video_range = ann['video_range']
        frame_start = max(ann['video_range'][0],idx-self.sample_interval)
        frame_end = min(ann['video_range'][1],idx+self.sample_interval)
        pair_idx = np.random.randint(frame_start, frame_end, size=1)[0]
        data_info_pair = super().get_data_info(pair_idx)
        data_info['timestamp_pair'] = data_info_pair['timestamp']
        assert data_info['video_id'] == data_info_pair['video_id']
        img_key_pair = data_info_pair['img_key']
        ann_pair = data_info_pair.pop('ann')
        ...
        if self.proposals is not None:
            if img_key not in self.proposals:
                ...
                data_info['proposals_pair'] = np.array([[0, 0, 1, 1]])
                data_info['scores_pair'] = np.array([1])
            else:
                ...
                proposals_pair = self.proposals[img_key_pair]
                assert proposals_pair.shape[-1] in [4, 5]
                if proposals_pair.shape[-1] == 5:
                    thr = min(self.person_det_score_thr, max(proposals_pair[:, 4]))
                    positive_inds = (proposals_pair[:, 4] >= thr)
                    proposals_pair = proposals_pair[positive_inds]
                    proposals_pair = proposals_pair[:self.num_max_proposals]
                    data_info['proposals_pair'] = proposals_pair[:, :4]
                    data_info['scores_pair'] = proposals_pair[:, 4]
                else:
                    proposals_pair = proposals_pair[:self.num_max_proposals]
                    data_info['proposals_pair'] = proposals_pair


        data_info['gt_bboxes'] = np.concatenate([ann['gt_bboxes'],ann_pair['gt_bboxes']],axis=0)
        data_info['gt_entity'] = np.concatenate([ann['entity_ids'],ann_pair['entity_ids']],axis=0)
        data_info['gt_labels'] = np.concatenate([ann['gt_labels'],ann_pair['gt_labels']],axis=0)
        data_info['instance_num'] = [ann['num_chimp'],ann_pair['num_chimp']]
        # print(f"{idx} dataset {data_info}")
