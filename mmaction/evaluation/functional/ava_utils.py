...
def results2csv(results, out_file, custom_classes=None, key='outputs'):
    """Convert detection results to csv file."""
    csv_results = det2csv(results, custom_classes,key=key)
...
def tpfp_single(tup, threshold=0.3):
        ...
        if iou.shape[1] > 0:
            ...
            for i in range(num_boxes):
                ...
                if iou[i, gt_id] >= threshold:
                    # print(f'IOOOOOU, {iou[i, gt_id]}, {threshold}, {label}')
...
def ava_eval(result_file,
    ...
    locomotion = np.nanmean([x[2] for x in cls_AP if 1 <= x[0] < 5])
