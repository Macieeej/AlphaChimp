...
def assign_process(data_sample: dict, pos_thr=0.5) -> dict:
    ...
    gt_box_indexes = list(range(len(gt_boxes)))
    gt_max_iou = [0] * len(gt_boxes)
    pred_max_iou = [0] * len(pred_boxes)

    unassigned_gt_labels = np.zeros_like(gt_labels)
    unassigned_pred_labels = np.zeros_like(pred_labels)

    assigned_pred_gt_labels = []
    for i, pred_box in enumerate(pred_boxes):
        max_iou = -1.0
        max_iou_idx = 0
        if len(gt_box_indexes) == 0:
            unassigned_pred_labels = pred_boxes[i:, :]
            break
        for j in gt_box_indexes:
            gt_box = gt_boxes[j]
            iou = iou_single_bbox(pred_box, gt_box)
            if iou > max_iou:
                max_iou = iou
                max_iou_idx = j
        gt_box_indexes.remove(max_iou_idx)

        pred_max_iou[i] = max_iou
        gt_max_iou[max_iou_idx] = max_iou
        assigned_pred_gt_labels.append((pred_labels[i], gt_labels[max_iou_idx], max_iou))

    if len(gt_box_indexes) != 0:
        unassigned_gt_labels = gt_labels[gt_box_indexes]

    pred_max_iou = np.array(pred_max_iou)
    gt_max_iou = np.array(gt_max_iou)

    return {'pred_gt_label_pairs': assigned_pred_gt_labels, 'unassigned_gt_labels': unassigned_gt_labels,
            'unassigned_pred_labels': unassigned_pred_labels, 'pred_scores': pred_scores, 'pred_ious': pred_max_iou, 'gt_ious': gt_max_iou}
...
def calculate_act_average_precision(processed_res, thr=0.2, num_classes=17) -> tuple:
    label_pairs = []
    ...
    match_ious = np.stack([x[2] for x in label_pairs])
    np_gt_labels = np.concatenate([x['unassigned_gt_labels'] for x in processed_res if x is not None], axis=0)
    np_pred_labels = np.concatenate([x['unassigned_pred_labels'] for x in processed_res if x is not None], axis=0)

    pred_labels = (pred_labels >= thr).astype(int)
    np_pred_labels = (np_pred_labels >= thr).astype(int)
    ...
    nps = np_gt_labels.sum(axis=0) + np_pred_labels.sum(axis=0)
...
class ChimpMetric4Class(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        pc_act, pc_w_act, rc_act, rc_w_act, tps, fps, tns, fns, nps, map_act, mar_act = (
            calculate_act_average_precision(results, self.threshold_act, len(self.action_class_names)))
