...
class FocalLossCost(BaseMatchCost):
    ...
    def _focal_loss_cost(self, cls_pred: Tensor, gt_labels: Tensor) -> Tensor:
        ...
        #gt_labels_bool = torch.sum(gt_labels, dim=0) >= 1
        #print(gt_labels.size())
        ...
        #    gt_label_bool = gt_label >= 0.5
        #    cls_cost_list.append(torch.sum(pos_cost[:, gt_label_bool] - neg_cost[:, gt_label_bool], dim=1))
        #cls_cost = torch.stack(cls_cost_list, dim=1)

        #cls_cost_list = []
        #for gt_label in gt_labels:
        #    gt_label_exp = gt_label
        #    if gt_label_exp.size(0) != cls_pred.size(1):
        #        gt_label_exp = torch.cat([torch.ones((1, ), device=gt_label_exp.device, dtype=gt_label_exp.dtype), gt_label_exp])
        #    gt_label_exp = gt_label_exp.unsqueeze(0).repeat(cls_pred.size(0), 1)
        #    loss_cls = self.bce_loss(cls_pred, gt_label_exp)
        #    loss_cls = loss_cls[:, 0] * self.obj_weight + loss_cls[:, 1:].mean(dim=1) * self.cls_weight
        #    cls_cost_list.append(loss_cls)

        #cls_cost = torch.stack(cls_cost_list, dim=1)

        if not self.no_obj_mode:
            obj_pred = cls_pred[:, 0].sigmoid()
            cls_cost_list = []
            for gt_label in gt_labels:
                cls_cost_list.append(-(obj_pred + self.eps).log() * self.alpha * (1 - obj_pred).pow(self.gamma))
            cls_cost = torch.stack(cls_cost_list, dim=1)
        else:
            cls_pred = cls_pred[:, 1:].sigmoid()
            neg_cost = -(1 - cls_pred + self.eps).log() * (
               1 - self.alpha) * cls_pred.pow(self.gamma)
            pos_cost = -(cls_pred + self.eps).log() * self.alpha * (
               1 - cls_pred).pow(self.gamma)
...
@TASK_UTILS.register_module()
class FocalLossCostHierarchy(BaseMatchCost):
    """FocalLossCost.
    ...
    def _focal_loss_cost(self, cls_pred: Tensor, gt_labels: Tensor) -> Tensor:
        ...
        Returns:
            torch.Tensor: cls_cost value with weight
        """
        if cls_pred.size(1) != self.num_classes + 1:
            cls_pred_fst = cls_pred[:, :self.num_classes + 1]
            if not self.subclass_no_cost:
                raise NotImplementedError
        else:
            cls_pred_fst = cls_pred

        cls_cost_list = []
        for gt_label in gt_labels:
            gt_label_exp = gt_label
            if gt_label_exp.size(0) != cls_pred_fst.size(1):
                gt_label_exp = torch.cat([torch.ones((1, ), device=gt_label_exp.device, dtype=gt_label_exp.dtype), gt_label_exp])
            gt_label_exp = gt_label_exp.unsqueeze(0).repeat(cls_pred_fst.size(0), 1)
            loss_cls = self.bce_loss(cls_pred_fst, gt_label_exp)
            loss_cls = loss_cls[:, 0] * self.obj_weight + loss_cls[:, 1:].mean(dim=1) * self.cls_weight
            cls_cost_list.append(loss_cls)
